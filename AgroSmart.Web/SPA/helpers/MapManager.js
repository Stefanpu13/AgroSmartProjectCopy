...
var mapManager = (function () {
    ...
    function initializeMenu(map, menuContainerId, viewMode) {
        if (viewMode === 'edit') {
            eidtableMenu = new menu.Menu(map, undefined, mediator);
            menu.create(viewMode, menuContainerId);
    ...
    function createMap(elementId, mainContainerId,viewMode, mapObjects) {
        mediator.reset();
        // first, init map.
        map = new google.maps.Map(document.getElementById(elementId));
        mapBuilder = new MapBuilder(map, defaultMapOptions, mediator, mapObjects, viewMode);
        mapBuilderEventsAttached = true;
        eidtableMenu = initializeMenu(map, mainContainerId, viewMode);
        initMenuItems = false;
    }
    ...
    function updateLocation(unitId, lat, lng) {
        // If user tries to see 'Mobile Module' and has no tasks.
        // The 'mapmanager' will start updating location without mapBuilder.        
        if (mapBuilder !== undefined) {
