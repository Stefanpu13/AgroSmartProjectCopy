...
var MapBuilder = (function (maps) {
    ...
    function MapBuilder(map, mapOptions, eventMediator, mapObjects, viewMode) {
        ...
        restoreMapObjects.call(this, mapObjects, viewMode);
    ...
    function restoreMapObjects(mapObjects, viewMode) {
        // 'mapObjects' can be single object or an array of objects.
        var regions = mapObjects.regions ||
            // flatten array of arrays to single array
            mapObjects.reduce(function (prevMo, currMo) {
                return prevMo.concat(currMo.regions);
            }, []);

        this.regions = this.restoreRegions(regions, mapManager.regionStatus.RESTORED);

        if (viewMode !== 'edit') {
            var units = mapObjects.units ||
            // Here objects are pushed to an array
            mapObjects.reduce(function (prevMo, currMo) {
                return prevMo.concat(currMo.units);
            }, []);

            this.units = this.restoreUnits(units);
        }
    }
