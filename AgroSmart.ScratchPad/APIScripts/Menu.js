...
var menu = (function (maps) {
        ...
        menuItems = (function () {
            button = (function () {
                ...
                var blockButton = (function () {
                    ...
                    function create(buttonAction) {
                        ...
                        switch (buttonAction) {
                            ...
                            case 'build':
                                ...
                                break;
                            case 'edit':
                                button = new Button('Edit block info', [{
                                    type: 'click', handler: publish('editButtonClicked')
                                }, {
                                    type: 'editBlockInfo', handler: showBlockWindow
                                }, {
                                    type: 'blockSelected', handler: show
                                }, {
                                    type: 'blockUnselected editingModeOff', handler: hide
                    ...
                    function showBlockWindow(blockInfo) {
                        var dialogForm = $("#dialog-form"),
                            name = dialogForm.find('#name'),
                            description = dialogForm.find('#descritpion'),
                            minBlockNameLength = 3,
                            blockInfo,
                            submitButtonName,
                            eventPublished;

                        // If block is edited then it current data should be displayed.
                        if (blockInfo !== undefined) {
                            name.val(blockInfo.name);
                            description.val(blockInfo.description);
                            submitButtonName = "Update Block";
                            eventPublished = 'blockUpdated';
                        } else { // It is new block - reset 'block info' form.
                            name.val('');
                            description.val('');
                            submitButtonName = "Create Block";
                            eventPublished = 'blockCreated pointUnselected';
                        }
                        ...
                        dialogForm.dialog({
                            ...
                            position: 'center',
                            buttons: [{
                                text: submitButtonName, click: function submitBlockInfo() {
                                    if (name.val().length < minBlockNameLength) {
                                        name.addClass("ui-state-error");
                                        displayError("Block name must be at least " + minBlockNameLength + " characters long!");

                                    } else {
                                        blockInfo = { name: name.val(), description: description.val() };
                                        mediator.publish(eventPublished, blockInfo);
                                        $(this).dialog("close");
                                    }
                                }
                            }]
                ...
                var globalButton = (function () {
                    ...
                    function toggleEditingMode() {
                        ...
                        if (editingModeOn) {
                            ...
                            mediator.publish(editingModeOffEvent + ' pointUnselected blockUnselected', { markerOptions: { draggable: false } });
    ...
    function createEditModeMenu() {
        ...
        var startEditingButton = menuItems.create('button.global.edit'),
            ...
            editBlockButton = menuItems.create('button.block.edit'),
